...
- The dashboard is served by a Python server located at /platform/server.py.
- The server loads data from /platform/elevator_fleet.csv generated by prepare_data.py.
- HTMX endpoints must return HTML fragments (not JSON); no custom JavaScript is used for table interactivity.