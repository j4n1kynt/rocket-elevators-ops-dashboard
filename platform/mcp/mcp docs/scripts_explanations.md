...
**Why it exists:**
`rag_tools.py` needs to search ChromaDB, but it should not manage the ChromaDB client or the embedding model directly. `rag.py` encapsulates both as lazy singletons — the ~125 MB SentenceTransformer model is loaded once on first use and reused for every subsequent query.
...
| `EMBEDDING_MODEL` | `BAAI/bge-small-en-v1.5` | Must match the model used to generate the stored embeddings |
