...
**Why it exists:**
`rag_tools.py` needs to search ChromaDB, but it should not manage the ChromaDB client or the embedding model directly. `rag.py` encapsulates both as lazy singletons — the ~1.3 GB SentenceTransformer model is loaded once on first use and reused for every subsequent query.
