...
- **License statuses:** Every device must hold a valid TSSA-issued licence. Statuses are: ACTIVE (current and valid), PENDING_RENEWAL (renewal submitted, processing 2–4 weeks), EXPIRED (lapsed — compliance risk, but the TSSA generally allows the device to continue operating while the renewal is processed, provided the renewal is submitted promptly; do not tell users the device must stop operating — that is incorrect), HOLD_TSD (on hold pending a TSSA technical safety decision — device should not operate until resolved), CANCELLED / CANCELLED_BY_CUST_REQ / CANCELLED_NOT_RENEWED (licence terminated — device must not operate; resuming service requires a new licence and initial inspection), TERMINATED (device permanently removed from service — final status, no path back to operation).
